...
export default function RemoveLiquidity({
  ...
  async function onRemove() {
    if (!chainId || !library || !account || godwokenAddress == null) throw new Error('missing dependencies')
    ...
    // TODO: maybe try eth_call to determine removeLiquidityETH or removeLiquidityETHSupportingFeeOnTransferTokens?
                      ...
                      {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                        <RowBetween style={{ justifyContent: 'flex-end' }}>
                          {oneCurrencyIsETH ? (
                            ...
                            >
                              {TranslateString(1188, 'Receive WCKB')}
                            </StyledInternalLink>
                          ) : oneCurrencyIsWETH ? (
                            <StyledInternalLink
                              to={`/liquidity/remove/${
                                currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'CKB' : currencyIdA
                              }/${currencyB && currencyEquals(currencyB, WETH[chainId]) ? 'CKB' : currencyIdB}`}
                            >
                              {TranslateString(1190, 'Receive CKB')}
                            </StyledInternalLink>
                          ) : null}
                        </RowBetween>
                      ) : null}
