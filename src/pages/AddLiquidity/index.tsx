...
import { getPairKey, getStablePoolByPairKey, IStablePool } from 'constants/stablePools'
...
export default function AddLiquidity({
  ...
  async function onAdd() {
    if (!chainId || !library || !account || godwokenAddress == null) return
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/liquidity/add/${currencyIdA || 'CKB'}/${newCurrencyIdB}`)
      }
    },
  ...
  useEffect(() => {
    ...
    const stablePool = getStablePoolByPairKey(chainId, getPairKey(currencyA, currencyB))
    if (stablePool != null) {
      history.push(`/liquidity/add/stable-pool/${stablePool.address}`)
    }
  }, [currencyA, currencyB, chainId, history])
        ...
        <Wrapper>
          ...
          <CardBody>
            <AutoColumn gap="20px">
              ...
              <PoolSelectPanel id="select-stable-pool-input" onPoolSelect={handlePoolSelect} />
              <AutoColumn gap="sm">
                <CurrencyInputPanel
