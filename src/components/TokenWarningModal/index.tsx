    ...
    <Wrapper error={duplicateNameOrSymbol}>
      <AutoRow gap="6px">
        ...
        <AutoColumn gap="10px" justify="flex-start">
          ...
          <Text title={token.address}>{shortenAddress(token.address)}</Text>
          {/* {chainId && (
      ...
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          ...
          <Text>
            {TranslateString(
              1130,
              'Anyone can create a ERC20 token on Godwoken Polyjuice with any name, including creating fake versions of existing tokens and tokens that claim to represent projects that do not have a token.'
            )}
          </Text>
          <Text>
            {TranslateString(
